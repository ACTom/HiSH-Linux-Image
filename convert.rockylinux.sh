...
# 检查参数数量
if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <source.wsl> <target.qcow2>"
    exit 1
fi
...
# Step 5: 解压 WSL 根文件系统到挂载目录
echo "Extracting WSL rootfs to mounted directory..."
tar -xzf "$SOURCE" -C "$MOUNT_DIR"
if [ $? -ne 0 ]; then
    echo "Failed to extract WSL rootfs."
    umount "$MOUNT_DIR"
    qemu-nbd -d "$NBD_DEVICE"
    rm -f "$QCOW2_FILE"
    rm -rf "$MOUNT_DIR"
...
# Step 10: 禁用 kernel 和 kernel-core 的更新
...
if [ -f "$DNF_CONF" ]; then
    ...
    echo "  Disabled kernel updates in DNF configuration"
else
